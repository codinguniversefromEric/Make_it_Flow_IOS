...
pattern = re.compile(r'    // MARK: - 2\. 閱讀順序演算法 \(XY-Cut / Projection Profile\).*?return finalSortedBlocks\n    }\n\n    /// 在一個水平大區塊 \(Region\) 內，區分出左右欄位並進行排序\n    private nonisolated static func sortRegionColumns.*?(?=\n    // MARK: - Style Mapping)', re.DOTALL)
content, count = pattern.subn(new_sort_layout, content)
print(f"Patched sortLayoutBlocks: {count}")
