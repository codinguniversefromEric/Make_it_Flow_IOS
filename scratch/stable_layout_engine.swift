...
enum LayoutEngine: Sendable {
    ...
    nonisolated static func processWithLayoutBlocks(
        ...
        // 如果完全沒有 YOLO block，直接走 fallback 流程保留所有文字
        if blocks.isEmpty {
            return fallbackLayout(fragments: fragments, pageWidth: pageWidth, pageHeight: pageHeight)
        }
        ...
        // 4. 對未被 YOLO 框選的碎片使用 fallback 幾何組裝
    ...
    // MARK: - Geometry Fallback Layout
    
    /// Fallback layout algorithm that groups fragments by geometric proximity (Y then X)
    private nonisolated static func fallbackLayout(fragments: [TextFragment], pageWidth: CGFloat, pageHeight: CGFloat) -> [ParagraphBlock] {
        guard !fragments.isEmpty else { return [] }
        
        // 1. 依 Y 座標排序
        let ySorted = fragments.sorted { $0.bounds.minY < $1.bounds.minY }
        
        // 2. 將 Y 座標相近的碎片分組為行 (Line)
        var lines: [[TextFragment]] = []
        var currentLine: [TextFragment] = [ySorted[0]]
        var currentLineMaxY = ySorted[0].bounds.maxY
        var currentLineMidY = ySorted[0].bounds.midY
        
        for i in 1..<ySorted.count {
            let frag = ySorted[i]
            // 如果片段的中心點落在目前行的高度範圍內，或是 Y 軸差距很小，視為同一行
            let yGap = frag.bounds.minY - currentLineMaxY
            
            if yGap > 5.0 && frag.bounds.midY > currentLineMaxY {
                lines.append(currentLine)
                currentLine = [frag]
                currentLineMaxY = frag.bounds.maxY
                currentLineMidY = frag.bounds.midY
            } else {
                currentLine.append(frag)
                currentLineMaxY = max(currentLineMaxY, frag.bounds.maxY)
                // 更新平均中心點
                currentLineMidY = (currentLineMidY * CGFloat(currentLine.count - 1) + frag.bounds.midY) / CGFloat(currentLine.count)
            }
        }
        if !currentLine.isEmpty { lines.append(currentLine) }
        
        // 3. 對每行內部的碎片進行 X 軸排序，並將相鄰行聚合成段落 (Paragraph)
        var paragraphs: [[TextFragment]] = []
        var currentParagraph: [TextFragment] = []
        var prevLineMaxY: CGFloat = -1
        let paragraphBreakGap: CGFloat = max(pageHeight * 0.02, 15.0) // 合理的段落間距
        
        for line in lines {
            let xSortedLine = line.sorted { $0.bounds.minX < $1.bounds.minX }
            let lineMinY = xSortedLine.map { $0.bounds.minY }.min()!
            
            if prevLineMaxY >= 0 && (lineMinY - prevLineMaxY) > paragraphBreakGap {
                if !currentParagraph.isEmpty {
                    paragraphs.append(currentParagraph)
                    currentParagraph = []
                }
            }
            currentParagraph.append(contentsOf: xSortedLine)
            prevLineMaxY = xSortedLine.map { $0.bounds.maxY }.max()!
        }
        if !currentParagraph.isEmpty { paragraphs.append(currentParagraph) }
        
        // 4. 將每個群組轉換為 ParagraphBlock
        let blocks = paragraphs.map { frags in
            buildParagraphBlock(from: frags, role: .body)
        }
        ...
        return blocks.filter { block in
            ...
            if isAtTop || isAtBottom {
                ...
                // 特徵 C: 短字串且包含疑似頁碼的數字結尾/開頭 (如 "ABOUT THE AUTHOR 241" 或 "241 ABOUT THE AUTHOR" 或 "62 · THE BOOK")
                if text.count < 80 {
    ...
    private nonisolated static func sortParagraphBlocks(_ blocks: [ParagraphBlock], pageWidth: CGFloat, pageHeight: CGFloat) -> [ParagraphBlock] {
        ...
        let regionBreakGap = max(pageHeight * 0.05, 30.0) // 使用與高度相關的比例，而非 pageWidth
        ...
        for region in regions {
            ...
            for i in 1..<xSorted.count {
                ...
                let isOverlapping = curr.bounds.minX < (currentMaxX - 20.0)
                
                // 判斷是否為新欄位：無水平重疊、X 跳躍距離過大，且前一欄寬度合理
    ...
    private nonisolated static func buildParagraphBlock(from fragments: [TextFragment], role: SemanticRole) -> ParagraphBlock {
        ...
        // 論文特化 OCR 錯誤修復：必須直接修改 fragments 的 text，因為 toHTML 是迭代 fragments
        let sanitizedFragments = fragments.map { frag in
            TextFragment(
        ...
        // 斷字修復：直接在 fragments 層級處理，否則 toHTML 仍會印出斷字與多餘空白
        var finalFragments: [TextFragment] = []
        for frag in sanitizedFragments {
            let trimmed = frag.text.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !trimmed.isEmpty else { continue }
            
            if let lastFrag = finalFragments.last, lastFrag.text.hasSuffix("-"),
               let lastChar = lastFrag.text.dropLast().last, lastChar.isLetter {
                // 移除上一個 fragment 的連字號，並將當前 fragment 附加過去
                let newText = String(lastFrag.text.dropLast()) + trimmed
                
                // 繼承上一個 fragment 的樣式與幾何，但擴展 bounds
                let mergedBounds = lastFrag.bounds.union(frag.bounds)
                let mergedFrag = TextFragment(
                    text: newText,
                    bounds: mergedBounds,
                    fontSize: lastFrag.fontSize,
                    fontName: lastFrag.fontName,
                    isBold: lastFrag.isBold,
                    isItalic: lastFrag.isItalic,
                    colorHex: lastFrag.colorHex
                )
                finalFragments[finalFragments.count - 1] = mergedFrag
            } else {
                // 若沒有發生斷字，則更新其 text 為 trimmed 版本
                let updatedFrag = TextFragment(
                    text: trimmed,
                    bounds: frag.bounds,
                    fontSize: frag.fontSize,
                    fontName: frag.fontName,
                ...
                finalFragments.append(updatedFrag)
            }
        }

        let unified = finalFragments.map { $0.text }.joined(separator: " ")
